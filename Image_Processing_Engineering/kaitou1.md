...
ラプラシアンフィルタの導出に入る前に、注目画素とその周囲8画素の位置関係を明確に示しておきます。以下の3×3行列は、中心の画素を $f(i,j)$ としたときの周囲画素の座標を示しています（ここで $i$ は水平方向のインデックスで右へ増加、$j$ は垂直方向のインデックスで下へ増加すると仮定します）。
...
この行列の読み方の例:
- 左上は $f(i-1, j-1)$。
- 右は $f(i+1, j)$（中心の画素 $f(i,j)$ の右隣）。
- 下は $f(i, j+1)$（中心の画素の1ピクセル下）。

この表記を使うことで、後の差分（例えば $f(i+1,j)-2f(i,j)+f(i-1,j)$ など）がどの画素に対応するかが一目で分かります。

### 4. ステップ1： 8近傍ラプラシアンフィルタ ($W_L$) の導出
