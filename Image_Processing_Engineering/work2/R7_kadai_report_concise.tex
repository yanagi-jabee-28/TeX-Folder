...
\textbf{Q=80} でSSIM=0.783、\textbf{Q=90} でSSIM=0.825 と向上するが、サイズ比は 11.8\%→17.7\%へ増加。視覚・数値の両面から、実用的には \textbf{Q=80～90} が妥当。\textbf{Q=100} はSSIM向上が小さい一方でサイズ比 45.5\% と非効率。 ただし、視覚的には \textbf{Q=10} の画像でも許容できると判断される場合がある（\autoref{fig:jpeg_q_grid}を参照）。SSIMは0.520と低めのため数値的には劣るが、用途によっては容量削減を優先して \textbf{Q=10} を採用することは妥当である。
