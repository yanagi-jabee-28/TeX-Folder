...
\paragraph{凸レンズBによる最終的な像の計算}
先に求めた中間像を，凸レンズBの物体と考える．
...
\paragraph{8近傍ラプラシアンフィルタ ($W_L$) の導出}
8近傍ラプラシアン $\nabla^2 f$ は，これら4方向すべての2次差分の総和として定義される．
\[
\nabla^2 f = f_{xx} + f_{yy} + f_{d1} + f_{d2}
\]
上で導出した4つの式をすべて足し合わせると，注目画素 $f(i,j)$ の係数は-8，周囲8画素の係数はすべて1となる．この係数を3x3の行列に配置することで，8近傍ラプラシアンフィルタ $W_L$ が得られる．
\[
W_L =
\begin{pmatrix}
1 & 1 & 1 \\
