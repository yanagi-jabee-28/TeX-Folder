...
\end{tcolorbox}
\begin{itemize}
    \item \textbf{真空準位 (Vacuum Level)}: 電子が原子から完全に自由になり、静止している状態のエネルギー（基準点 $0$）。
    \item \textbf{全障壁 $W$ (Potential Barrier)}: 金属の底から真空準位までの高さ。
    \item \textbf{フェルミ準位 $\EF$ (Fermi Level)}: 金属内の電子が詰まっている「水面」の高さ。
    \item \textbf{仕事関数 $\phi$ (Work Function)}: フェルミ準位（水面）にある電子を、外（真空準位）に引っ張り出すのに必要な「追加コスト」。
    \item \textbf{位置エネルギー (Potential Energy)}: 真空準位を基準($E=0$)として、金属内部に束縛された電子は\textbf{負のポテンシャルエネルギー}を持ちます。電子の全エネルギーは運動エネルギー$K$と位置エネルギー$U$の和で表され、
    \begin{equation}
        E_{\mathrm{total}} = K + U
    \end{equation}
    真空準位(0)より高くなると外部へ脱出できます（$E_{\mathrm{total}}\ge 0$）。
    \item \textbf{外部エネルギー (External Energy)}: 熱・光・電界・電子衝突などの外部から与えられるエネルギーで、電子の運動エネルギーを増加させます。外部エネルギー$\Delta E_{\mathrm{ext}}$を与えた結果、放出条件は
    \begin{equation}
        E_{\mathrm{total}} + \Delta E_{\mathrm{ext}} \ge 0
    \end{equation}
    であり、フェルミ準位近傍から見れば光電子放出の場合は$h\nu \ge e\phi$などで表されます。
\end{itemize}
...
\subsection*{⑩ 熱陰極の具備条件}
良いエミッタ（陰極材料）であるための3条件：
\begin{enumerate}
    \item \textbf{仕事関数 $\phi$ が小さいこと}: 低い温度でも多くの電子を放出できるため。
    \item \textbf{融点が高いこと}: 高温動作しても溶けないようにするため。
    \item \textbf{寿命が長いこと}: 高温でも蒸発しにくく、安定していること。
\end{enumerate}
※代表例：タングステン(W)、酸化物陰極(BaO/SrO)。
