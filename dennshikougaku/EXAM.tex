% Use LuaLaTeX (luatex) with bxjsarticle; don't specify dvipdfmx here
\documentclass[a4paper,11pt]{bxjsarticle}
% Use fontspec + luatexja for LuaLaTeX (handles Japanese glyphs correctly)
\usepackage{fontspec}
\usepackage{luatexja-fontspec}
% If you prefer specific fonts, uncomment and set as appropriate:
...
\usepackage{tcolorbox}    % 枠線用
\usepackage{url}

% 余白の設定: reset class geometry first, then set explicit A4 paper and margins.
% Using `reset` clears class defaults and avoids over-specification warnings.
\geometry{reset, a4paper, left=25mm, right=25mm, top=30mm, bottom=30mm}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{フェルミ準位 ($E_F$)}: 絶対零度（0K）で電子が詰まっている最大のエネルギー準位。
    \item \textbf{仕事関数 ($\phi$)}: 電子を固体表面から真空中に取り出すのに必要な最小エネルギー。
    \begin{itemize}
        \item 数式定義：$\phi = W - E_F$ （真空準位 $W$ とフェルミ準位の差）
...
\begin{tcolorbox}[colback=white,colframe=black,title=\textbf{直前対策まとめ}]
