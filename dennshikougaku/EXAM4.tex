...
%==============================================================================
% 制作：試験対策委員会
% 内容：試験範囲ポイント①〜⑲の完全網羅解説 ＋ 2024/2023年度過去問詳細解答 ＋ レポート課題演習
%==============================================================================
...
\author{ポイント①〜⑲網羅解説 \& 過去問・レポート課題詳細解法}
\date{}
...
\subsection*{⑲ 電界の強さと電位、ベクトル表記、ポアソン・ラプラスの方程式}
\textbf{【記述試験・レポート課題4 対応重要項目】}

空間内の電位 $V$ と電界 $\bm{E}$ の間には、微分（勾配）の関係があります。

\subsubsection*{1. 電位 $V$ と電界 $\bm{E}$ のベクトル関係}
電界ベクトル $\bm{E}$ は、電位 $V$ の勾配（Gradient）の逆符号で定義されます。電位が高い方から低い方へ向かう傾きが電界の向きです。
\begin{tcolorbox}[colback=white, colframe=red, title=\textbf{重要公式}]
\[ \bm{E} = -\nabla V = -\text{grad } V \]
成分表示（直交座標系）では：
\[ \bm{E} = -\left( \frac{\partial V}{\partial x}\bm{i} + \frac{\partial V}{\partial y}\bm{j} + \frac{\partial V}{\partial z}\bm{k} \right) \]
\end{tcolorbox}
※ $\nabla$（ナブラ）はベクトル微分演算子 $\nabla = (\frac{\partial}{\partial x}, \frac{\partial}{\partial y}, \frac{\partial}{\partial z})$ です。
レポート課題4のように $V$ が座標の関数で与えられた場合、各成分を偏微分して符号を変えることで電界を求めます。

\subsubsection*{2. 支配方程式（ポアソン・ラプラス）}
電位 $V$ は空間内の電荷分布 $\rho$ によって決定されます。
\begin{itemize}
    \item \textbf{ポアソンの方程式}（電荷 $\rho$ がある場合）:
    \[ \nabla^2 V = \frac{\partial^2 V}{\partial x^2} + \frac{\partial^2 V}{\partial y^2} + \frac{\partial^2 V}{\partial z^2} = -\frac{\rho}{\varepsilon_0} \]
    1次元モデル（極板間など）では $\frac{\dd^2 V}{\dd x^2} = -\frac{\rho}{\varepsilon_0}$ として積分計算に用います。
    \item \textbf{ラプラスの方程式}（電荷がない場合 $\rho=0$）:
    \[ \nabla^2 V = 0 \]
\end{itemize}

\subsubsection*{3. 計算手順のまとめ（1次元の場合）}
記述問題で「電界と電位を求めよ」と問われた場合の定石です。
\begin{enumerate}
    \item \textbf{出発点}: ポアソン方程式 $\frac{\dd^2 V}{\dd x^2} = -\frac{\rho}{\varepsilon_0}$
    \item \textbf{第1積分}: $x$ で積分し、電界 $E_x = -\frac{\dd V}{\dd x}$ を求める（積分定数 $C_1$ 出現）。
    \item \textbf{第2積分}: もう一度積分し、電位 $V$ を求める（積分定数 $C_2$ 出現）。
    \item \textbf{境界条件}: 与えられた条件（例：$x=0, V=0$）を代入し、$C_1, C_2$ を決定する。
\end{enumerate}
...
%==============================================================================
提出レポート課題として課された問題の詳細解答です。試験でも同様の数値計算や記述（特に課題4のベクトル演算）が出題される可能性が高いため、プロセスを完璧に理解してください。
...
\end{align*}
※試験では $A = 1.20 \times 10^6$ が与えられることが一般的ですが、導出を問われる可能性もあるため式と値を覚えておきましょう。
...
\textbf{解答プロセス:}
ポイント⑲で解説した通り、電界は電位の勾配（gradient）の逆符号です。
...
※ ベクトル表記 $(x, y, z)$ や $\bm{r} / r^3$ といった表記でも正解となります。
