...
% パッケージ設定
%------------------------------------------------------------------------------
\usepackage{lmodern}              % 【修正】フォントサイズ警告を解消（スケーラブルフォント）
...
% エラーログで推奨された修正：\qtyコマンドをsiunitxの\SIとして上書き定義
...
% \unitコマンドの定義（siunitxと競合しないよう、ドキュメント開始時に再定義）
% 目的：単位を [eV] のように角括弧で囲むスタイルにするため
% 以下はより堅牢な定義方法です。
% - siunitx が \unit を定義している場合はそれを \siunitxUnit にバックアップ
% - そうでない場合は Fallback を用意します（linter/エディタ向け）
\AtBeginDocument{%
    % siunitx の \unit が存在するならバックアップする
    \ifcsname unit\endcsname
        % copy current meaning: \siunitxUnit -> \unit
        \expandafter\let\csname siunitxUnit\endcsname\unit
    \else
        % fallback 定義（存在しない場合に備える）
        \providecommand{\siunitxUnit}[1]{\,[\mathrm{#1}]}%
    \fi
    % ドキュメント全体で使う簡易な \unit を定義（角括弧表記）
