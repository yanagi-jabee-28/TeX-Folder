...
\clearpage
% ===================================================================
\chapter{伝搬定数の厳密な導出と損失分離}
% ===================================================================
あなたの研究の根幹をなす式 $\alpha = \alpha_c + \alpha_d$ は，実は「低損失」という条件下で成立する近似式です．ここでは，伝送線路の基本方程式から出発し，近似を一切含まない厳密な減衰定数 $\alpha$ の一般式を導出します．その上で，低損失近似を適用することで，実用的な分離式がどのように導かれるのかを解説します．

\section{伝送線路方程式（電信方程式）}
伝送線路上の電圧 $V(z)$ と電流 $I(z)$ の振る舞いは，線路の微小区間 $\Delta z$ における等価回路から導かれます．キルヒホッフの法則を適用し，$\Delta z \to 0$ の極限をとることで，以下の連立微分方程式が得られます．これを電信方程式（Telegrapher's Equations）と呼びます．
\begin{align}
...
\section{波動方程式と伝搬定数\texorpdfstring{$\gamma$}{gamma}}
上記2式の片方を $z$ で微分し，もう一方を代入して整理すると，電圧と電流がそれぞれ独立して満たすべき波動方程式が得られます．
\begin{align}
    \frac{\dd^2 V(z)}{\dd z^2} - (R + \mi\omega L)(G + \mi\omega C) V(z) &= 0 \\
    \frac{\dd^2 I(z)}{\dd z^2} - (R + \mi\omega L)(G + \mi\omega C) I(z) &= 0
\end{align}
ここで，複素数である伝搬定数 $\gamma$ を以下のように定義します．
\begin{equation}
    \gamma^2 = (R + \mi\omega L)(G + \mi\omega C)
\end{equation}
すると，波動方程式は $\frac{\dd^2 V(z)}{\dd z^2} - \gamma^2 V(z) = 0$ と簡潔に書け，その一般解は進行波と後進波の重ね合わせ $V(z) = V_1 e^{-\gamma z} + V_2 e^{+\gamma z}$ となります．$\gamma$ の実数部が減衰を，虚数部が位相の回転を司ることがわかります．

\section{減衰定数\texorpdfstring{$\alpha$}{alpha}と位相定数\texorpdfstring{$\beta$}{beta}の厳密な導出}
伝搬定数 $\gamma$ を実数部の減衰定数 $\alpha$ と虚数部の位相定数 $\beta$ に分離して $\gamma = \alpha + \mi\beta$ とおき，$\alpha$ と $\beta$ の厳密な一般式を導出します．

まず，$\gamma^2$ を展開し，実数部と虚数部に分離します．
\begin{equation}
    \gamma^2 = (RG - \omega^2 LC) + \mi\omega(RC + GL)
\end{equation}
一方で，$(\alpha + \mi\beta)^2$ を展開すると，
\begin{equation}
    (\alpha + \mi\beta)^2 = (\alpha^2 - \beta^2) + \mi(2\alpha\beta)
\end{equation}
両辺の実数部と虚数部を比較することで，以下の連立方程式が得られます．
\begin{align}
    \alpha^2 - \beta^2 &= RG - \omega^2 LC \label{eq:real_part} \\
    2\alpha\beta &= \omega(RC + GL) \label{eq:imag_part}
\end{align}
式 \eqref{eq:imag_part} から $\beta = \omega(RC + GL) / (2\alpha)$ を式 \eqref{eq:real_part} に代入します．
\begin{equation}
    \alpha^2 - \left( \frac{\omega(RC + GL)}{2\alpha} \right)^2 = RG - \omega^2 LC
\end{equation}
両辺に $4\alpha^2$ を掛けて整理すると，$\alpha^2$ に関する二次方程式（複二次方程式）が得られます．
\begin{equation}
    4\alpha^4 - 4(RG - \omega^2 LC)\alpha^2 - \omega^2(RC + GL)^2 = 0
\end{equation}
これを $\alpha^2$ について解の公式を用いて解くと（$\alpha^2 > 0$ であるため正の解のみ採用），
\begin{equation}
    \alpha^2 = \frac{4(RG - \omega^2 LC) + \sqrt{16(RG - \omega^2 LC)^2 + 16\omega^2(RC + GL)^2}}{8}
\end{equation}
平方根の中を整理すると，$(R^2+\omega^2L^2)(G^2+\omega^2C^2) = (RG-\omega^2LC)^2 + \omega^2(RC+GL)^2$ という恒等式が成り立つため，
\begin{equation}
    \alpha^2 = \frac{1}{2} \left\{ (RG - \omega^2 LC) + \sqrt{(R^2+\omega^2L^2)(G^2+\omega^2C^2)} \right\}
\end{equation}
したがって，減衰定数 $\alpha$ の厳密な一般式は以下となります．
\begin{equation}
    \alpha = \sqrt{\frac{1}{2} \left\{ \sqrt{(R^2+\omega^2L^2)(G^2+\omega^2C^2)} + (RG - \omega^2 LC) \right\}}
\end{equation}
同様に，$\beta$ についても解くことができ，以下の厳密式が得られます．
\begin{equation}
    \beta = \sqrt{\frac{1}{2} \left\{ \sqrt{(R^2+\omega^2L^2)(G^2+\omega^2C^2)} - (RG - \omega^2 LC) \right\}}
\end{equation}

\section{低損失近似と損失分離}
前節で導出した厳密式は複雑ですが，「低損失条件」（$R \ll \omega L, G \ll \omega C$）を適用することで，物理的に見通しの良い形に近似できます．
この条件下で $\alpha$ の厳密式をテイラー展開すると，
\begin{equation}
    \alpha \approx \frac{R}{2} \sqrt{\frac{C}{L}} + \frac{G}{2} \sqrt{\frac{L}{C}}
\end{equation}
となり，低損失線路の特性インピーダンス $Z_0 \approx \sqrt{L/C}$ を用いると，
\begin{equation}
    \alpha \approx \frac{R}{2 Z_0} + \frac{G Z_0}{2}
\end{equation}
この式は，全体の減衰 $\alpha$ が，導体の物理特性($R$)のみに依存する項と，誘電体の物理特性($G$)のみに依存する項の単純な和で表せることを示しています．したがって，導体損失 $\alpha_c$ と誘電損失 $\alpha_d$ は，この低損失近似の下で，以下のように定義されます．
\begin{align}
    \alpha_c &\equiv \frac{R}{2 Z_0} \label{eq:alpha_c_def_re} \\
    \alpha_d &\equiv \frac{G Z_0}{2} \label{eq:alpha_d_def_re}
\end{align}

\section{損失の周波数特性の導出}
この分離された定義式から，各損失成分の周波数依存性を導出します．
\begin{itemize}
    \item \textbf{導体損失 $\alpha_c$ の周波数特性:}
    $\alpha_c$ の定義式 \eqref{eq:alpha_c_def_re} に含まれる高周波抵抗 $R$ は，表面抵抗 $R_s$ を用いて $R \propto R_s$ とモデル化できます．さらに $R_s = \sqrt{\pi f \mu / \sigma}$ であるため，$R \propto \sqrt{f}$ となります．したがって，
    \begin{equation}
        \alpha_c \propto R \propto \sqrt{f}
    \end{equation}
    より具体的には，$\alpha_c = K_c \sqrt{f}$ と書け，比例定数 $K_c$ は線路形状と導体物性（$\sigma, \mu$）で決まります．

    \item \textbf{誘電損失 $\alpha_d$ の周波数特性:}
    $\alpha_d$ の定義式 \eqref{eq:alpha_d_def_re} に含まれるコンダクタンス $G$ は，$G = \omega C \tan\delta = 2\pi f C \tan\delta$ とモデル化できます．したがって，$G \propto f$ となります．
    \begin{equation}
        \alpha_d \propto G \propto f
    \end{equation}
    より具体的には，$\alpha_d = K_d f$ と書け，比例定数 $K_d$ は線路形状と誘電体物性（$\epsilon_r, \tan\delta$）で決まります．
\end{itemize}
...
\section{Sパラメータの正式な数学的定義と正規化}
VNAや回路解析で扱うSパラメータは，電圧や電流ではなく，\textbf{正規化された波} $a_n$ と $b_n$ を用いて定義されます．これらの波は「単位電力」を基準に正規化され，ポート $n$ における入射電力が $|a_n|^2$，反射(出力)電力が $|b_n|^2$ になるように定義されます．

一般に，ポート$n$での瞬時電圧$V_n$と電流$I_n$が与えられるとき，基準インピーダンス $Z_{0n}$ に対する入射波と反射波は以下のように定義されます（この定義はKeysightや他ベンダーのドキュメントと整合します）．
\begin{align}
    a_n &= \frac{V_n + Z_{0n} I_n}{2\sqrt{\operatorname{Re}(Z_{0n})}} \\
    b_n &= \frac{V_n - Z_{0n} I_n}{2\sqrt{\operatorname{Re}(Z_{0n})}}
\end{align}
ここで $Z_{0n}$ はポート $n$ の参照インピーダンス（通常は50\,\Omega）で，複素数になる場合もあります．上式の目的は，定義により $|a_n|^2$ と $|b_n|^2$ がそれぞれ入射・反射の平均電力となることを保証する点にあります．

すべてのポートについて $\boldsymbol{a}=[a_1, a_2, \dots]^\intercal$，$\boldsymbol{b}=[b_1, b_2, \dots]^\intercal$ を導入すると，Sパラメータは行列として
\begin{equation}
    \boldsymbol{b} = \boldsymbol{S} \, \boldsymbol{a}
\end{equation}
と書けます．ここで $\boldsymbol{S}$ は $N$ ポートネットワークに対する $N\times N$ 散乱行列です．

\section{2ポートネットワークのS行列と物理的意味}
最も一般的な2ポートネットワークでは，
\begin{equation}
    \begin{pmatrix} b_1 \\
                  b_2 \end{pmatrix}
    = \begin{pmatrix} S_{11} & S_{12} \\
      ...
      \begin{pmatrix} a_1 \\
                  a_2 \end{pmatrix} .
\end{equation}
前に述べたように，各成分の物理的意味は「\textbf{ポート$j$に入射させたとき，ポート$i$から出てくる波の比}」です．測定時には，入射していない全ポートを整合終端（$a_n=0$）にして定義を評価します．

例：
\begin{itemize}
    \item $S_{11} = b_1 / a_1\big|_{a_2=0}$ : 入力反射係数（入力整合の良さ）
    \item $S_{21} = b_2 / a_1\big|_{a_2=0}$ : 順方向透過係数（挿入損失・ゲイン）
    \item $S_{22} = b_2 / a_2\big|_{a_1=0}$ : 出力反射係数
    \item $S_{12} = b_1 / a_2\big|_{a_1=0}$ : 逆方向透過係数
\end{itemize}

\section{Sパラメータの複素表現と物理的解釈}
Sパラメータは複素数であり，振幅と位相の両方の情報を持ちます．一般に
\begin{equation}
    S_{ij} = |S_{ij}| e^{\mi\angle S_{ij}} = \operatorname{Re}(S_{ij}) + \mi \operatorname{Im}(S_{ij})
\end{equation}
と書けます．振幅 $|S_{ij}|$ は電力伝達や反射率を表し，位相 $\angle S_{ij}$ は伝搬時間や遅延に対応します．

例えば $|S_{21}|$ のデシベル表示（挿入損失）は
\begin{equation}
    	ext{Insertion Loss [dB]} = -20 \log_{10} |S_{21}|
\end{equation}
と定義されるのが一般的です（能動デバイスのゲインは正の値となるため，この関数は通常符号規約に依存します）．

\section{Sパラメータの重要な性質}
\paragraph{相互性（Reciprocity）:}
線形で受動的な材料（相互作用を持つが非磁性，非アクティブなデバイス）では，散乱行列は対称行列になります：$S_{ij} = S_{ji}$．これはネットワークの相互性（受動対称性）に由来する重要な性質です（ただし，非相互材料や磁性体，アクティブデバイスでは成立しません）．

\paragraph{エネルギー保存（無損失）:}
損失のない（理想的な）受動ネットワークでは散乱行列はユニタリ（単位行列を保つ）です．つまり
\begin{equation}
    \boldsymbol{S}^\dagger \boldsymbol{S} = \boldsymbol{I}
\end{equation}
が成り立ちます．ここで $\dagger$ は随伴行列（複素共役転置）を示します．この条件は，全てのポートにおける入射電力と出射電力が等しいことを意味します．

\paragraph{パッシブ性}:
パッシブなネットワークでは，任意の入射波に対して出力波の総電力は入射電力以下となるため，行列ノルムから $|S_{ii}| \le 1$ 等の不等式が導かれます（対角成分は，適切に整合された場合はさらに小さくなります）．

\section{測定上の注意点：基準インピーダンスと較正（Calibration）}
VNAで測定したSパラメータは，必ず\textbf{基準インピーダンス}（通常は $Z_0=50\,\Omega$）に対して規格化された値です．この参照を変える（例えば50\,\Omegaから75\,\Omegaへ）と，数値としてのSパラメータは変化します．また，実際の測定ではVNAとDUTの間に存在するケーブルやコネクタ，方向性結合器の誤差がS値に混入するため\textbf{校正（Calibration）}が必須です．

主な較正方式としては，SOLT（Short-Open-Load-Thru），TRL（Thru-Reflect-Line），LRM（Line-Reflect-Match）などがあり，それぞれ適用場面と精度特性が異なります．KeysightやAnritsuのアプリノートは，\textbf{VNAエラーモデル}（直接波，反射波，伝送誤差の分離）と較正の実際手順について詳細に説明しています（参照：Keysight Application Notes）．

\subsection{実践的な注意点（BCDR測定への適用）}
共振器測定（BCDRや他の共振器）においては，以下の点に特に注意してください：
\begin{itemize}
    \item \textbf{ダイナミックレンジ（ダイナミック・レンジ）:} 共振周波数付近ではS21が相対的に大きく，それ以外は\SI{-60}{\deci\bel}程度の非常に小さな値となることがあります．VNAのダイナミックレンジは，測定可能な最小S21のレベルを決めるため，十分なダイナミックレンジが必要です（低ノイズの受信機，低損失ケーブル，適切な出力パワー設定）．
    \item \textbf{IF帯域幅とスイープ時間:} ノイズ対策のためIF帯域幅（IFBW）を狭く設定すると，ノイズが低減される反面スイープ時間が長くなります．低レベルの透過（高減衰）を精密測定する場合は，IFBWを小さくし平均化をかけることが有効です（Keysightアプリノート参照）．
    \item \textbf{ポート挿入損失・ターミネート:} 測定の参照基準面（Reference Plane）は校正手順で正しく定義する必要があります．コネクタやアダプタで参照面を移動する場合は，ポート延長（port extension）やデエンベデッド（de-embedding）を使って補正してください．
    \item \textbf{サンプリング密度:} 共振ピークの幅が非常に狭い場合は，周波数点の分解能（ポイント数）を高くすることで，正確な\(f_r\) と \(\Delta f\) の読み取りが可能になります．
\end{itemize}

\section{Sパラメータから他パラメータへの変換（簡単な概要）}
Sパラメータは，Zパラメータ，Yパラメータ，ABCDパラメータ（チェーンパラメータ）などと相互に変換が可能です．例えば2ポートのABCD行列 $\begin{pmatrix} A & B \\\ C & D \end{pmatrix}$ とS行列は，参照インピーダンス $Z_0$ を用いて変換できます．詳細な式は教科書に譲りますが，伝送線路系の解析やカスケード（直列結合）した回路性能の解析ではABCDを経由してSに戻す操作が実用的です．

\subsection{反射係数と負荷インピーダンスの関係}
ポートにおける反射係数（帰還係数）$\Gamma$ は，負荷インピーダンス $Z_L$ と基準インピーダンス $Z_0$ の比として
\begin{equation}
    \Gamma = \frac{Z_L - Z_0}{Z_L + Z_0}
\end{equation}
で与えられます．完全整合（$Z_L = Z_0$）では $\Gamma=0$，完全反射（短絡または開放）では $|\Gamma|=1$ になります．理想的かつ単一ポートの場合，反射係数 $\Gamma$ はそのまま $S_{11}$ に対応しますが，一般の2ポートネットワークでは，ソース側の整合状態や逆方向の透過が影響するため，注意が必要です．

Smithチャートは，複素数の反射係数 $\Gamma$ を直感的に可視化する強力なツールであり，インピーダンス整合の設計やSパラメータの視覚的解析で広く用いられます．
...
第3章で理論的に導出したように，導体損失 $\alpha_c$ は周波数の平方根 $\sqrt{f}$ に，誘電損失 $\alpha_d$ は周波数 $f$ に比例します．この2つの損失成分は，周波数に対して異なる振る舞いを示します．
\begin{align}
    \alpha_c(f) &= K_c \sqrt{f} \\
    \alpha_d(f) &= K_d f
\end{align}
ここで $K_c, K_d$ は材料と線路形状で決まる比例定数です．
...
このグラフから，「高周波帯では誘電損失が支配的になる」という重要な結論が導かれます．周波数 $f$ が十分に高くなると，線形に増加する $\alpha_d$ が，緩やかに増加する $\alpha_c$ を必ず追い越すためです．例えばFR-4（丸印）を見ると，25 GHzでは誘電損失 $\alpha_d$（約 \SI{7.5}{\deci\bel/100mm}）が，導体損失 $\alpha_c$（約 \SI{1.5}{\deci\bel/100mm}）を遥かに上回っていることが一目瞭然です．
