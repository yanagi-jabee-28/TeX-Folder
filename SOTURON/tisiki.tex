...
導体損失 $\alpha_c$ がどのように導電率 $\sigma$ と結びついているのか，物理法則から段階的に説明します．

\begin{enumerate}
    \item \textbf{出発点：伝送線路の減衰定数}
    伝送線路理論によれば，低損失線路における導体が原因の減衰定数 $\alpha_c$ は，単位長さあたりの抵抗 $R$ と線路の特性インピーダンス $Z_0$ を用いて，次のように表されます．
    \begin{equation}
        \alpha_c \approx \frac{R}{2 Z_0}
    \end{equation}
    この式は，単位長さあたりの抵抗 $R$ が大きいほど，損失が大きくなるという物理的直感に一致します．

    \item \textbf{高周波における抵抗 $R$ のモデル化}
    高周波では「表皮効果」により，電流は導体の表面近傍に集中します．このときの単位面積あたりの抵抗が「表面抵抗 $R_s$」[\si{\ohm\per\text{sq.}}]です．
    電流が幅 $W$ の導体に流れると考えると，単位長さあたりの抵抗 $R$ は，この表面抵抗 $R_s$ を幅 $W$ で割ることで近似できます．（ここでは電流が信号線とグラウンドの両方を流れることを考慮し，実効的な抵抗としてモデル化します）
    \begin{equation}
        R \propto \frac{R_s}{W}
    \end{equation}
    これを最初の式に代入すると，$\alpha_c$ は表面抵抗 $R_s$ に比例し，線路幅 $W$ に反比例する関係が見えてきます．
    \begin{equation}
        \alpha_c \propto \frac{R_s}{Z_0 W}
    \end{equation}
    これは，ご提示の式 `$\alpha_c = R_s / (Z_0 W)$` が示す物理的関係の本質です．

    \item \textbf{表面抵抗 $R_s$ と導電率 $\sigma$ の関係}
    表面抵抗 $R_s$ は，電流が流れる層の厚さである「表皮深さ $\delta_s$」と導体の「導電率 $\sigma$」で決まります．物理的には，$R_s = 1 / (\sigma \delta_s)$ という関係にあります．
    ここに，第2章で示した $\delta_s = 1/\sqrt{\pi f \mu \sigma}$ の式を代入すると，
    \begin{equation}
        R_s = \frac{1}{\sigma \delta_s} = \frac{1}{\sigma} \sqrt{\pi f \mu \sigma} = \sqrt{\frac{\pi f \mu}{\sigma}}
    \end{equation}
    となり，表面抵抗 $R_s$ は，周波数 $f$ の平方根に比例し，導電率 $\sigma$ の平方根に反比例することがわかります．
    ...
    \end{equation}
    これは物理的に「\textbf{導電率 $\sigma$ が高い材料（＝電気が流れやすい良い導体）ほど，導体損失 $\alpha_c$ は小さくなる}」という直感に一致する重要な結果です．
\end{enumerate}
