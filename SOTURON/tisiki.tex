...
\clearpage
% ===================================================================
\chapter{損失の分離と周波数依存性の導出}
% ===================================================================
あなたの研究の根幹をなす式 $\alpha = \alpha_c + \alpha_d$ は，実は「低損失」という条件下で成立する近似式です．ここでは，近似を含まない厳密な式から出発し，どのようにしてこの実用的な分離式が導かれるのか，そしてその過程で $\alpha_c$ と $\alpha_d$ の周波数依存性がどのように決まるのかを解説します．
...
\section{導体損失\texorpdfstring{$\alpha_c$}{alpha c}の周波数特性}
式 \eqref{eq:alpha_c_def} で定義された導体損失 $\alpha_c$ が，どのように周波数 $f$ と関係するのかを導出します．
\begin{enumerate}
    \item \textbf{高周波抵抗 $R$ のモデル化:} 式 \eqref{eq:alpha_c_def} の $R$ は，表皮効果を考慮した高周波抵抗です．これは表面抵抗 $R_s$ [\si{\ohm\per sq.}] を用いて，$R \approx R_s / W_{\mathrm{eff}}$ とモデル化できます．ここで $W_{\mathrm{eff}}$ は電流が流れる実効的な幅です．
    \begin{equation}
        \alpha_c \approx \frac{R_s}{2 Z_0 W_{\mathrm{eff}}}
    \end{equation}
    \item \textbf{表面抵抗 $R_s$ の周波数依存性:} 第2章で示した通り，表面抵抗 $R_s$ は周波数 $f$ を含む以下の式で与えられます．
    \begin{equation}
        R_s = \sqrt{\frac{\pi f \mu}{\sigma}} = \left( \sqrt{\frac{\pi \mu}{\sigma}} \right) \sqrt{f}
    \end{equation}
    \item \textbf{最終形:} 上記の2式を組み合わせることで，$\alpha_c$ の周波数依存性が陽に示された式が得られます．
    \begin{equation}
        \alpha_c \approx \left( \frac{1}{2 Z_0 W_{\mathrm{eff}}} \sqrt{\frac{\pi \mu}{\sigma}} \right) \sqrt{f}
    \end{equation}
    この式は，$\alpha_c = K_c \sqrt{f}$ の形をしています．ここで比例定数 $K_c$ は，線路の形状 ($Z_0, W_{\mathrm{eff}}$) と導体の材料物性 ($\mu, \sigma$) のみで決まる，周波数に依存しない係数です．
    \begin{equation}
        \alpha_c = K_c \sqrt{f}, \quad \text{ただし} \quad K_c = \frac{1}{2 Z_0 W_{\mathrm{eff}}} \sqrt{\frac{\pi \mu}{\sigma}}
    \end{equation}
    これにより，\textbf{導体損失 $\alpha_c$ は周波数の平方根に比例する}ことが理論的に示されます．
\end{enumerate}

\section{誘電損失\texorpdfstring{$\alpha_d$}{alpha d}の周波数特性}
同様に，式 \eqref{eq:alpha_d_def} で定義された誘電損失 $\alpha_d$ の周波数依存性を導出します．
\begin{enumerate}
    \item \textbf{コンダクタンス $G$ のモデル化:} 式 \eqref{eq:alpha_d_def} の $G$ は，誘電体の漏れ電流を表し，複素誘電率の議論から $G = \omega C \tan\delta$ とモデル化できます．
    \begin{equation}
        \alpha_d = \frac{(\omega C \tan\delta) Z_0}{2} = \frac{(2\pi f C \tan\delta) Z_0}{2} = (\pi C Z_0 \tan\delta) f
    \end{equation}
    \item \textbf{物理定数による書き換え:} ここで，$Z_0 \approx \sqrt{L/C}$ と，線路の位相速度 $v_p \approx 1/\sqrt{LC}$ の関係を用いると，$C Z_0 \approx C \sqrt{L/C} = \sqrt{LC} = 1/v_p$ となります．さらに，$v_p$ は光速 $c$ と実効比誘電率 $\epsilon_{\mathrm{eff}}$ を用いて $v_p = c/\sqrt{\epsilon_{\mathrm{eff}}}$ と表せます．
    \begin{equation}
        \alpha_d \approx \pi \left( \frac{1}{v_p} \right) \tan\delta \cdot f = \frac{\pi \sqrt{\epsilon_{\mathrm{eff}}}}{c} \tan\delta \cdot f
    \end{equation}
    \item \textbf{最終形:} この式は，$\alpha_d = K_d f$ の形をしています．ここで比例定数 $K_d$ は，材料の誘電特性 ($\epsilon_{\mathrm{eff}}, \tan\delta$) のみで決まる係数です（ここでは $\tan\delta$ 等の周波数依存性は無視）．
    \begin{equation}
        \alpha_d = K_d f, \quad \text{ただし} \quad K_d = \frac{\pi \sqrt{\epsilon_{\mathrm{eff}}}}{c} \tan\delta
    \end{equation}
    これにより，\textbf{誘電損失 $\alpha_d$ は周波数に直接比例する}ことが理論的に示されます．
\end{enumerate}
