...
% ===================================================================
\chapter{測定の基本原理：Sパラメータ}
% ===================================================================

\section{なぜSパラメータが必要か？ — 高周波測定の課題}
マイクロ波やミリ波といった高周波領域では、回路の「電圧」や「電流」を直接測定することが極めて困難になります。その理由は以下の通りです。
\begin{itemize}
    \item \textbf{理想的な測定条件の不存在:} 回路の特性を測るための基準となる「完全な短絡（ショート）」や「完全な開放（オープン）」を物理的に作ることができません。開放端はアンテナとして機能し電磁波を放射してしまい、短絡端もわずかな配線がインダクタンス成分を持ってしまいます。
    \item \textbf{プロービングの影響:} 測定プローブを回路に接触させる行為そのものが、回路の特性インピーダンスを乱し、正確な測定を妨げます。
\end{itemize}
この問題を解決するため、高周波領域では電圧や電流の代わりに、伝送線路を伝わる電力の流れを「波」として捉え、その振る舞いを記述するSパラメータ（Scattering Parameters, 散乱行列）が用いられます。

\section{Sパラメータの物理的・数学的定義}

\subsection{電力波の導入：入射波 ($a_n$) と出射波 ($b_n$)}
Sパラメータは、被測定物（DUT: Device Under Test）の各接続点（ポート）における電力波 (Power Wave) の関係性を記述します。電力波とは、その振幅の2乗が電力の次元を持つように正規化された波の表現です。

\begin{description}
    \item[入射波 ($a_n$)] ポート$n$に向かって進む波であり、測定器がDUTに「入力する」刺激（Stimulus）に相当します。VNAからDUTへ供給される基準信号がこれにあたります。$|a_n|^2$はポート$n$への入射電力を表します。
    \item[出射波 ($b_n$)] ポート$n$から出ていく波であり、入射波に対するDUTの「応答」（Response）に相当します。DUTで反射された波や、DUTを透過してきた波がこれにあたります。$|b_n|^2$はポート$n$からの出射電力を表します。
\end{description}

\subsection{2ポートネットワークのSパラメータ行列}
最も一般的な2ポートネットワーク（入力と出力が1つずつある回路）では、入力である入射波ベクトル $\bm{a}$ と、出力である出射波ベクトル $\bm{b}$ の関係は、Sパラメータ行列 $\bm{S}$ を用いて以下のように線形関係で結ばれます。
\begin{equation}
...
\end{equation}
これは、以下の2つの連立方程式と等価です。
\begin{align*}
    b_1 &= S_{11}a_1 + S_{12}a_2 \\
    b_2 &= S_{21}a_1 + S_{22}a_2
\end{align*}

\subsection{各Sパラメータ要素の物理的意味}
各Sパラメータの要素 $S_{ij}$ は、「ポート$j$に波を入射させたとき、ポート$i$からどれだけの波が出てくるか」を示す複素数です。この定義を正確にするため、測定時には信号を入射させるポート以外はすべて、その系の特性インピーダンス（例: \SI{50}{\ohm}）で完全に整合終端します。整合終端されたポートでは、外部からの反射波が存在しないため、そのポートへの入射波はゼロ ($a_n=0$) と見なせます。

\begin{description}
    \item[$S_{11}$ (入力反射係数)] ポート2を整合終端 ($a_2=0$) した状態で、ポート1に入射した波 $a_1$ のうち、ポート1に反射して戻ってきた波 $b_1$ の比率。
    $$ S_{11} = \frac{b_1}{a_1} \bigg|_{a_2=0} $$
    \item[$S_{21}$ (順方向透過係数)] ポート2を整合終端 ($a_2=0$) した状態で、ポート1に入射した波 $a_1$ のうち、ポート2を透過して出てきた波 $b_2$ の比率。
    $$ S_{21} = \frac{b_2}{a_1} \bigg|_{a_2=0} $$
    \item[$S_{22}$ (出力反射係数)] ポート1を整合終端 ($a_1=0$) した状態で、ポート2に入射した波 $a_2$ のうち、ポート2に反射して戻ってきた波 $b_2$ の比率。
    $$ S_{22} = \frac{b_2}{a_2} \bigg|_{a_1=0} $$
    \item[$S_{12}$ (逆方向透過係数)] ポート1を整合終端 ($a_1=0$) した状態で、ポート2に入射した波 $a_2$ のうち、ポート1を透過して出てきた波 $b_1$ の比率。
    $$ S_{12} = \frac{b_1}{a_2} \bigg|_{a_1=0} $$
\end{description}

\section{Sパラメータの複素数表現：振幅と位相}
Sパラメータは、単なる大きさの比ではなく、振幅と位相の両方の情報を持つ複素数です。VNAで測定すると、各周波数に対して実部と虚部、あるいは振幅と位相のペアとしてデータが得られます。
\begin{equation}
    S_{ij} = \mathrm{Re}(S_{ij}) + \mi \cdot \mathrm{Im}(S_{ij}) = |S_{ij}| e^{\mi\angle S_{ij}}
\end{equation}
\begin{itemize}
    \item \textbf{振幅 ($|S_{ij}|$)}:
    電力の伝達率や反射率を表します。デシベル (dB) 単位で表現されることが多く、例えば透過特性 $|S_{21}|$ は挿入損失 (Insertion Loss) として知られています。
    $$ \text{Insertion Loss [dB]} = -20 \log_{10} |S_{21}| $$
    （注：損失を表すため負号をつける定義が一般的です）
    \item \textbf{位相 ($\angle S_{ij}$)}:
    信号がDUTを通過または反射する際に生じる位相の遅れ（または進み）を表します。これは信号の伝搬時間（遅延）と直接関係しています。
\end{itemize}

\section{VNAによる測定原理}
VNA（ベクトルネットワークアナライザ）は、その名の通り、Sパラメータのベクトル量（振幅と位相）を測定するための装置です。VNAがS21の振幅と位相を測定する基本原理は「ヘテロダイン検波」に基づいています\cite{AllAboutCircuits:VNA}．
\begin{enumerate}
    \item \textbf{信号発生と分配:} VNA内部の信号源が，ある周波数 $f$ の高周波信号（基準信号）を発生させ，ポート1からDUTへ送出します．これが $a_1$ です\cite{Keysight:VNA}．
    \item \textbf{基準波の捕捉:} ポート1の直前にある方向性結合器が，$a_1$ のごく一部を分岐させ，VNA内部の基準受信機（R受信機）へ送ります\cite{Keysight:VNA}．
    \item \textbf{透過波の捕捉:} DUTを透過した信号 $b_2$ は，ポート2からVNAに入り，測定受信機（T受信機）へ送られます\cite{Anritsu:VNA}．
    \item \textbf{ダウンコンバート:} ギガヘルツの信号を直接扱うのは困難です．そこで，VNA内部の局部発振器（LO）からの信号と混合（Mix）し，R受信機とT受信機の両方で，低い中間周波数（IF）に周波数変換（ダウンコンバート）します\cite{AllAboutCircuits:VNA}．
