...
\noindent
\begin{tabular}{@{}ll}
  \BoldLabel{テーマ名} & \UnderlineBox[13.5cm]{自動制御実験} \\[2.0em] 
\end{tabular}
...
\section{目的}
2次遅れ系となる回路を作成し，その周波数応答を測定した。測定結果から伝達関数を算出し，理論値と比較することで，周波数応答と伝達関数の関係を理解することを目的とする。
...
\begin{table}[H]
    ...
    \begin{tabular}{@{} >{\centering\arraybackslash}m{0.8cm} >{\raggedright\arraybackslash}m{2.8cm} >{\raggedright\arraybackslash}m{4.0cm} >{\raggedright\arraybackslash}m{3.6cm}@{\hspace{2pt}} >{\raggedright\arraybackslash}m{3.0cm} @{} }
        ...
        2 & オシロスコープ & OWON / PDS5022S & $50\,\si{MHz}, 1\,\si{GS/s}$ & W5022S10030760 \\
        3 & 直流安定化電源 & AND / AD-8724D & $0 \sim 18\,\si{V}, 10\,\si{A}$ & 005153, 005156 \\
        \bottomrule
    \end{tabular}
    \endgroup
\end{table}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.95\linewidth}
        ...
        \caption{MATLAB による理論・シミュレーション（ボード線図）（報告事項③）}
        \label{fig:bode_sim}
    \end{subfigure}
    \\[1em]
    ...
    \end{subfigure}
    \caption{比較図：(a) MATLABシミュレーション（報告事項③）, (b) 実測ゲイン, (c) 実測位相（報告事項④）}
    \label{fig:bode_images}
\end{figure}

\section{考察}

\subsection{実験結果からの伝達関数の導出と検証（報告事項①・④）}
...
\section{感想}
感想としては，配線作業は共同実験者の倉科純太郎くんの協力がなければ完了し得なかった。その他の点では，グループ内の連携が良好であり，実験を円滑に進めることができた。
