...
実際の装置では、データとチャンネル信号を合成しているが、本実験ではその代わりにclock回路の出力T2とT3を用いてこの回路の動作を理解する。回路図は図7に示す。
...
図2のタイミングパルス発生回路を作成して過渡現象のシミュレーションを実行した結果を図11に示す。図11の結果より、T1はRCOが出力されるタイミングで反転している。また、前述の図2の回路の真理値表どおりに動作しており、理論と整合する。

### **5.2 切替回路**

図12に図3の切替回路を組んで過渡現象をシミュレーションした結果を掲げる。
Voutを見ると、Vclockのタイミングで100 [Hz]の信号と50 [Hz]の信号が交互に出力されていることが確認できる。以上より理論と整合する。

### **5.3 標本化回路（サンプル&ホールド回路）**

図13に図4の標本化回路を組んで過渡現象シミュレーションを実行した結果を示す。
Voutの波形を見ると、Vclockの立ち上がりタイミングでVG1の電圧に一致し、Vclockが0になってSW1がOFFになってからも電圧が維持されていることが確認できる。以上より理論と整合する。

### **5.4 シフトレジスタ（パラレルシリアル変換）**

図14に図6のシフトレジスタを組んでシミュレーションした過渡現象を示す。パラレル信号はSW-HL1, SW-HL6, SW-HL7, SW-HL8をHighとし、それ以外をLowとした。OUTPUTを見ると、T3が出力された後にHigh, High, High, Low, Low, Low, Low, Highが出力され、SW-HL8から順にパラレル信号が出力されている。よって理論と整合する。

### **5.5 波形合成・分離回路**

図15に図7の波形合成・分離回路を組んで過渡現象をシミュレーションした結果を示す。
Vmixを見ると、T2の波形はプラス、T3の波形はマイナスで合成されている。また、VT2にはT3と同じ形状の波形が、VT3にはT2と同じ形状の波形が出力され、両者の振幅は5.00 [V]である。したがって理論と整合する。

### **5.6 AD-DA変換回路、ローパスフィルタ**

図16に図9のAD-DA変換回路、ローパスフィルタを組んで過渡現象をシミュレーションした結果を示す。
Vinを見ると、VG1の波形が+6 [V]上にシフトされていることが確認できる。以上より理論と整合する。
VDAは、U1 74199でAD変換された信号をMV95308でアナログ信号に戻してV1で加えた6 [V]分を引いた後の波形である。振幅値をカーソルで確認すると、T2のクロックごとに振幅の値が離散的に変化している。したがって、T2のタイミングで入力されるデジタル信号に対応した電圧を出力しており、理論と整合する。
VQAとVQHについては、VG1の値が変化するごとに0 [V]と4 [V]のいずれかに変化している。よって理論と整合する。
V復調を見ると、VDAではT2のクロックごとに変化していた振幅が連続的に変化するようになった。また、最大振幅は-0.07 [V]を中心にとると5.00 [V]である。以上より理論と整合する。
