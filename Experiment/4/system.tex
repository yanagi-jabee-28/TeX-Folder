...
\end{equation}
理論的に共振ピーク値 $M_p$ は減衰係数 $\ze$ の関数として次式で与えられる。
\begin{equation}
    M_p = \frac{1}{2\ze\sqrt{1-\ze^2}}
\end{equation}
この式を $\ze$ について解くために両辺を2乗し，整理を行う。
\begin{equation}
    M_p^2 = \frac{1}{4\ze^2(1-\ze^2)} \iff 4M_p^2 \ze^4 - 4M_p^2 \ze^2 + 1 = 0
\end{equation}
これは $\ze^2$ に関する2次方程式とみなせる。解の公式を用いて $\ze^2$ を求める。
\begin{align}
...
\end{align}
ここで，共振現象が生じる条件は $\ze < 1/\sqrt{2}$ （すなわち $\ze^2 < 1/2$）である。$\sqrt{1 - 1/M_p^2} > 0$ であるため，複号のうち正（$+$）を選択すると $\ze^2 > 1/2$ となり条件を満たさない。したがって，複号は負（$-$）を選択する。
ゆえに，$\ze$ は次式で表される。
\begin{equation}
    \ze = \sqrt{\frac{1 - \sqrt{1 - \frac{1}{M_p^2}}}{2}}
\end{equation}
ここに実験値 $M_p = 1.973$ を代入すると，
\begin{equation}
