...
\begin{table}[H]
    ...
    % Removed \resizebox to preserve font size; use tabular* to stretch to text width without scaling
    \begingroup\normalsize\setlength{\tabcolsep}{1pt}%
    \setlength{\extrarowheight}{0pt}% make rows tight
    \renewcommand{\arraystretch}{1.0}% normal line spacing -- tighten row height
    % Use vertically centered fixed-width columns to reduce height and avoid overfull hbox
    \begin{tabular}{S[table-format=4.1] S[table-format=5.1] >{\centering\arraybackslash}m{1.2cm} >{\centering\arraybackslash}m{1.2cm} >{\centering\arraybackslash}m{1.2cm} S[table-format=2.2] >{\centering\arraybackslash}m{1.1cm} >{\centering\arraybackslash}m{1.6cm}}
        \toprule
