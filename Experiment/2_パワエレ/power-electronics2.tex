...
\section{実験方法}
実験系の概略図および計測系の結線図を図1に示す．詳細は取扱説明書に従い，実験を実施した．取得した測定データは保存し，後節に示す図\ref{fig:regeneration-results}，\ref{fig:current-control-results}，\ref{fig:speed-control-results}にまとめた．

\subsection{ACブラシレスモータのトルク制御}
\begin{enumerate}
    \item 機器を準備した後，軸継手を用いてモータ，トルク検出器，ブレーキを連結した（図1参照）．
...
\begin{enumerate}
    \item 機器を準備した後，軸継手を用いてモータとフライホイールを連結した（図1参照）．
