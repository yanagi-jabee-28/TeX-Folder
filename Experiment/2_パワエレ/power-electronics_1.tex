...
\begin{itemize}
    \item \textbf{ACサーボモータ}: ワコー技研 ANR020-C308（\unit{200W}，\unit{14.5A}，\unit{2000rpm}，8極，レゾルバ付）
    \item \textbf{フライホイール}: 昭和電業社（質量 \unit{17.65kg}，慣性モーメント \unit{0.07069kg \cdot m^2}）
    \item \textbf{ブレーキ}: 三菱電機 ZHA-20A（トルク \unit{2N \cdot m}，\unit{2000rpm}）
    \item \textbf{電気二重層コンデンサ}: 日本ケミコン MDLA15R0V116FB0（$\unit{38.6F} \times 3 = \unit{115.8F}$）
    \item \textbf{電源}: 昭和電業社（\unit{DC0 \sim 30V}，\unit{10A}，回生機能内蔵）
    \item \textbf{インバータ}: 昭和電業社（IPM使用）
    \item \textbf{計測装置}: 昭和電業社
    \item \textbf{ブレーキ用電源}: 昭和電業社（\unit{DC24V}，\unit{1A}）
    \item \textbf{トルク検出器}: 小野測器 SS-050（\unit{5N \cdot m}，\unit{6000rpm}，トルク表示器と組）
\end{itemize}

\subsection{電気自動車実習装置（主な構成品）}
\begin{itemize}
    \item \textbf{ACサーボモータ}: ワコー技研 B751E-D4R（\unit{48V}，\unit{750W}，\unit{21A}，\unit{2000rpm}，レゾルバ付）
    \item \textbf{バッテリー}: FIAMM 12SPX42（\unit{12V}，\unit{42Ah}）$\times 4$ 直列
\end{itemize}
...
\begin{enumerate}
    ...
    \item 電源装置の電圧を\unit{9V}に設定して，計測ソフトウェアの設定数値画面で「制御方法：電流制御（トルク制御）」，「出力変動方法：手動」，「一定制御時間：120秒」に設定して，「開始」ボタンを押した後に「電流」の数値を変化させることでモータの回転数が変化することを確認する。
...
\begin{enumerate}
    \item 電源装置の電圧を\unit{15V}に設定して，計測ソフトウェアの設定数値画面で「制御方法：速度制御」，「出力変動方法：手動」，「一定制御時間：120秒」に設定して，「開始」ボタンを押した後に「速度」の数値を変化させることでモータの回転数が変化することを確認する。また，回転させているときに「ブレーキ電流」の数値を変化させることでモータの回転数が変化することを確認する。
