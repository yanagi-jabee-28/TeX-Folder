...
% リンク・参照
\usepackage{cite}
% improve URL line breaks
\usepackage{xurl}
\usepackage[hidelinks]{hyperref}
\usepackage[nameinlink,noabbrev]{cleveref}
\usepackage{needspace}
% allow breaks within URLs to avoid large inter-word spacing
...
\begin{enumerate}
    ...
    この関係は，染谷―Shannon の標本化定理と呼ばれている．つまり，アナログ信号に含まれる最高周波数が $f_0$ であるとき，標本化周波数が $2f_0$ 以上あれば，標本化されたパルス波から，元のアナログ信号を再現できるということを示している（例えば，アナログ信号の最高周波数が \SI{4}{\kilo\hertz} であれば，標本化周波数は \SI{8}{\kilo\hertz} でよいことになる．）\cite{hatori2012}．
    ...
    したがって，インパルス列 $g(0)$，$g(1T)$，$g(2T)$，$\dots$ に対するフィルタの応答波形は，\cref{fig:sampling_theory}（c），（d），（e）に示すように，$h_s(t)$ を $T$ ずつ時間的にずらした波形に標本値振幅を掛け合わせたものになる．各標本値に対するこのような応答波形をすべて総和したものが，最終的な補間・復調された波形で，\cref{fig:sampling_theory}（f）のように連続波形に合成される．\cite{kaneko1977}

    \begin{figure}[H]
        \centering
        \includegraphics[max size={0.6\linewidth}{0.5\textheight},keepaspectratio]{image/img_016.png}
        \caption{標本化とその復調\cite{kaneko1977}}
        \label{fig:sampling_theory}
    \end{figure}

    \item \textbf{本実験における量子化レベルはいくらかを報告する．}
    
    量子化とは，標本化が時間的に離散的な信号抽出をするのに対し，振幅軸方向で離散的な値をとることである．\cite{kaneko1977}
    ...
    通常のシリコンのダイオードでは順方向降下電圧があるので，それ以上の電圧を加えないと電流が流れ始めない．一方で，理想ダイオードは順方向電圧では短絡，逆方向電圧では開放である特性を持つ．\cite{takasaki2021}

    \item \textbf{\cref{fig:ad_da_circuit}のローパスフィルタの遮断周波数はいくらかを報告する．\cite{electricalinfo2023,terashima2019}}
    ...
    \item \textbf{PCM 変調がほかの変調方式に比べて優れている点を述べる．\cite{kaneko1977}}
    ...
    受信する際は標本周期と同じタイミングで信号を受信すれば，正しく信号を受けることができる．\cite{kaneko1977}

    \begin{figure}[H]
        \centering
        \includegraphics[max size={0.6\linewidth}{0.45\textheight},keepaspectratio]{image/img_018.png}
        \caption{時分割多重化（TDM）\cite{kaneko1977}}
        \label{fig:tdm}
    \end{figure}

    \item \textbf{PCM 通信はどのようなところで用いられているかを簡単に説明する．}
    
    Bell 電話研究所の研究により，1962 年に T1 方式が世界初の商用化された PCM 通信として登場した．日本においても戦後いち早く日本電信電話公社電気通信研究所や大学の研究機関によって PCM の研究が開始された．1965 年には，24 回戦 PCM 方式が日本電信電話公社により商用化されるに至った．その後，高次群 PCM の開発，マイクロ PCM，同軸 PCM，ミリ波・準ミリ波通信，データ端局，画像符号化，統合網，衛星通信，さらには最近の光ファイバ通信などのように PCM 技術が用いられている．\cite{kaneko1977}
\end{enumerate}

\section{参考文献}
\begin{thebibliography}{9}
\raggedright
\bibitem{kaneko1977} 金子尚志：「PCM 通信の技術」，産報出版株式会社，1977．
\bibitem{hatori2012} 羽鳥光俊：「わかりやすい通信工学」，コロナ社，2012．
\bibitem{takasaki2021} 高崎和之：「基本からわかる電子回路」，株式会社ナツメ社，2021．
\bibitem{electricalinfo2023} 「Electrical Information」，\url{https://detail-infomation.com/sallen-key-low-pass-filter/}（参照 2023 年 12 月 14 日）．
\bibitem{terashima2019} 寺島一彦，兼重明宏：「制御工学 技術者のための，理論・設計から実装まで」，実教出版株式会社，2019．
\end{thebibliography}
