...
\usepackage{siunitx}
\usepackage{float}
% Better control of floats: avoid forcing them into place with [H] and reduce float pile-up
\usepackage{placeins}
\usepackage[margin=2.5cm]{geometry}
% Provide a safe maximum height for tall images and keep aspect ratio
\setkeys{Gin}{height=0.6\textheight,keepaspectratio}

% ページ下部の余白調整を緩める（Overfull \vbox 対策）
\raggedbottom
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_000.png}
  \caption{基本方式}
  \label{fig:1}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_001.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth,height=0.6\textheight,keepaspectratio]{image/img_002.png}
  \caption{切替回路}
  \label{fig:3}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{image/img_003.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_004.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{image/img_005.png}
  \caption{シフトレジスタ}
  \label{fig:6}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{image/img_006.png}
  \caption{波形合成・分離回路}
  \label{fig:7}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth,height=0.6\textheight,keepaspectratio]{image/img_007.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{image/img_008.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{image/img_009.png}
  \caption{マクロ用回路}
  \label{fig:10}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_010.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_011.png}
  \caption{切替回路の過渡特性}
  \label{fig:12}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_012.png}
  \caption{標本化回路の過渡特性}
  \label{fig:13}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_013.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_014.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.6\textheight,keepaspectratio]{image/img_015.png}
...
\end{equation}
この関係は，染谷―Shannonの標本化定理と呼ばれている．つまり，アナログ信号に含まれる最高周波数が$f_0$であるとき，標本化周波数が$2f_0$以上あれば，標本化されたパルス波から，元のアナログ信号を再現できるということを示している（例えば，アナログ信号の最高周波数が4 [\si{\kilo\hertz}]であれば，標本化周波数は8 [\si{\kilo\hertz}]でよいことになる．）\texorpdfstring{\cite{hatori2012}}{}．
...
したがって，インパルス列$g(0), g(1T), g(2T), \cdots$に対するフィルタの応答波形は，図17（c），（d），（e）に示すように，$h_s(t)$をTずつ時間的にずらした波形に標本値振幅を掛け合わせたものになる．各標本値に対するこのような応答波形をすべて総和したものが，最終的な補間・復調された波形で，図17（f）のように連続波形に合成される．\texorpdfstring{\cite{kaneko1977_1}}{}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth,height=0.6\textheight,keepaspectratio]{image/img_016.png}
  \caption{標本化とその復調}
  \label{fig:17}
\end{figure}

\subsection{(2) 本実験における量子化レベルはいくらかを報告する．}
量子化とは，標本化が時間的に離散的な信号抽出をするのに対し，振幅軸方向で離散的な値をとることである．\texorpdfstring{\cite{kaneko1977_2}}{}
...
通常のシリコンのダイオードでは順方向降下電圧があるので，それ以上の電圧を加えないと電流が流れ始めない．一方で，理想ダイオードは順方向電圧では短絡，逆方向電圧では解放である特性を持つ．\texorpdfstring{\cite{takasaki2021}}{}

\subsection{(4) 図9のローパスフィルタの遮断周波数はいくらかを報告する．\texorpdfstring{\cite{elec-info, terashima2019}}{}}
図9のローパスフィルタ（LPF）はサレン・キー型LPFと呼ばれるもので，オペアンプと抵抗R，コンデンサC，で構成されている2次LPFである．入力電圧VINの低周波成分を通過させ，高周波成分を遮断する．

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{image/img_017.png}
...
\subsection{(5) PCM変調がほかの変調方式に比べて優れている点を述べる．\texorpdfstring{\cite{kaneko1977_3}}{}}
...
受信する際は標本周期と同じタイミングで信号を受信すれば，正しく信号を受けることができる．\texorpdfstring{\cite{kaneko1977_4}}{}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{image/img_018.png}
  \caption{時分割多重化（TDM）}
  \label{fig:19}
\end{figure}

\subsection{(7) PCM通信はどのようなところで用いられているかを簡単に説明する．}
Bell電話研究所の研究により，1962年にT1方式が世界初の商用化されたPCM通信として登場した．日本においても戦後いち早く日本電信電話公社電気通信研究所や大学の研究機関によってPCMの研究が開始された．1965年には，24回線PCM方式が日本電信電話公社により商用化されるに至った．その後，高次群PCMの開発，マイクロPCM，同軸PCM，ミリ波・準ミリ波通信，データ端局，画像符号化，統合網，衛星通信，さらには最近の光ファイバ通信などのようにPCM技術が用いられている．\texorpdfstring{\cite{kaneko1977_5}}{}
