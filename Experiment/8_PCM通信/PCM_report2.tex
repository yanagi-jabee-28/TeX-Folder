...
% ----- 画像・単位・レイアウト関連 -----
% 【重要】[dvipdfmx]オプションを削除しました。
% コンパイラの自動判定に任せることで、lualatex/platexの不整合を防ぎます。
\usepackage{graphicx} 
% adjustbox: provides 'max size' option for \includegraphics (prevents upscaling small images)
...
% 画像の高さ制限設定（keepaspectratio のみをデフォルトにし、個々の図では max size=.. を指定する）
\setkeys{Gin}{keepaspectratio}

% --- 画像品質に関するメモ（編集中に参照） -----
% 1) 回路図やベクトル描画は可能な限りベクターファイル（.pdf, .eps）で出力し,
%    LaTeXに埋め込んでください（拡大しても画質劣化しません）。
% 2) スクリーンショットやラスター画像（.png/.jpg）は印刷品質のために
%    300dpi（目安）以上で出力・保存してください。細い線（回路図など）は
%    600dpi推奨です。
% 3) 小さなラスター画像を
%    \includegraphics[width=0.6\textwidth]{...}
%    のように強制拡大すると画質が大きく劣化します。本ファイルでは
%    `adjustbox` の `max size` を用いて「最大表示サイズ」を指定しており,
%    小さい画像は拡大せず，最大サイズを超える画像だけ縮小する設定です。
% 4) TINA-TIなどのシミュレータからは可能ならPDF出力（ベクター）を使い,
%    できない場合はPNGの出力解像度を上げてください。
...
% ----- ハイパーリンク -----
% リンクの色を黒に統一（図番号などの参照箇所の色付けを黒にする）
...
この式の物理的意味を以下に説明する．入力信号 $g(t)$ を無限小のパルス幅を持つインパルス $\delta(t)$ の列で標本化すると，標本化パルス列 $g_T(t)$ は次式で表される（図\ref{fig:17}(b)）．
\begin{equation}
...
\end{equation}
で与えられる理想的な低域通過フィルタに通すと，その出力信号 $y(t)$ は畳み込み積分により
\begin{equation}
  y(t) = \int_{-\infty}^{\infty} g_T(\tau) h_s(t-\tau)d\tau = \sum_{n=-\infty}^{\infty} g(nT) \cdot h_s(t - nT) \label{eq:4}
\end{equation}
となり，式(\ref{eq:1})と等価になる．これは，各標本点におけるインパルス応答（Sinc関数）の重ね合わせによって元の波形が滑らかに補間・復調されることを意味する（図\ref{fig:17}(c)～(f)）\texorpdfstring{\cite{kaneko1977_1}}{}．

\begin{figure}[H]
  \centering
  \includegraphics[max size={0.6\textwidth}{0.45\textheight}]{image/img_016.png}
  \caption{標本化とその復調の原理}
  \label{fig:17}
\end{figure}
...
本実験で使用したローパスフィルタ（図\ref{fig:9}の末段）は，オペアンプ，抵抗，コンデンサで構成されるサレン・キー（Sallen-Key）型2次LPFである（図\ref{fig:18}）．
...
図\ref{fig:18}においてキルヒホッフの法則を適用し，伝達関数を導出する．各部の電流および電圧の関係は以下の通りである．
\begin{align}
  I_1 &= \frac{V_{IN} - V_x}{Z_1}, \quad I_2 = \frac{V_{OUT} - V_x}{Z_2} \label{eq:5} \\
  I_3 &= \frac{V_x}{Z_3 + Z_4} = \frac{V_{OUT}}{Z_4} \label{eq:7}
\end{align}
オペアンプの入力インピーダンスが無限大であると仮定すると $I_3 = I_1 + I_2$ が成立する．式(\ref{eq:7})より $V_x = \frac{Z_3 + Z_4}{Z_4} V_{OUT}$ となる．これらを整理して伝達関数 $G(s) = V_{OUT}/V_{IN}$ を求めると，最終的に以下の形式が得られる．
\begin{equation}
...
標本化定理に基づき，各チャンネルの信号をパルス列としてサンプリングする際，それぞれのパルスのタイミング（位相）を互いに重ならないようにずらすことで，時間軸上で複数の信号をインターリーブ（多重化）することが可能となる（図\ref{fig:19}）．受信側では，送信側と同期をとることで，特定のタイムスロットの信号のみを分離して取り出し，各チャンネルの信号を復元する\texorpdfstring{\cite{kaneko1977_4}}{}．

\begin{figure}[H]
  \centering
  \includegraphics[max size={0.7\textwidth}{0.45\textheight}]{image/img_018.png}
  \caption{時分割多重（TDM）の概念図}
  \label{fig:19}
\end{figure}
