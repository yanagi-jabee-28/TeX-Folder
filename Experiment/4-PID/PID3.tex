...
% =============================================
% 1. フォント・数式・記号 (Conflict Free)
% =============================================
\usepackage[T1]{fontenc}
\usepackage{newtxtext}
\usepackage[varbb]{newtxmath} % amssymb等の機能も内包（競合回避のためamssymbは削除推奨）
\usepackage{bm}      % ベクトル太字 (\bm)
\usepackage{mathtools} % amsmathの拡張版（dcases等が使える）

% --- 【賢い機能】微分記号の自動化 ---
% \diff{y}{x} -> dy/dx, \pdiff{y}{x} -> ∂y/∂x を定義
...
\usepackage{graphicx}
\usepackage{float}
\usepackage{booktabs}  % 美しい罫線 (\toprule, \midrule, \bottomrule)
\usepackage{subcaption}

% --- 【賢い機能】SI単位系の最新規格対応 (v3) ---
\usepackage{siunitx}
\sisetup{
  detect-all,           % 本文フォントに合わせる
  inter-unit-product=\ensuremath{{}\cdot{}}, % 単位積をドットで表現 (N・m)
  separate-uncertainty=true % 誤差表記 (±) 対応
}

% --- 【賢い機能】グラフ描画 (TikZ/PGFPlots) ---
\usepackage{tikz}
\usepackage{pgfplots}
\pgfplotsset{compat=newest} % 常に最新の描画エンジンを使用
\usetikzlibrary{arrows.meta, positioning, calc}

% =============================================
% 3. 相互参照・リンク・引用 (Automation)
% =============================================
% 引用を上付き[1]にする（手動定義をやめて専用パッケージを使う）
\usepackage[superscript]{cite}

% ハイパーリンク（印刷用に枠を消し、色は黒に統一）
\usepackage[hidelinks]{hyperref}

% --- 【賢い機能】参照の完全自動化 (cleveref) ---
% \cref{fig:aaa} と書くだけで「図1」と自動出力。番号変更も追従。
...
\usepackage{caption}
\captionsetup{
  format=hang,          % 複数行になった時のインデントを揃える
  labelsep=quad,        % ラベルと本文の間を全角空白に
  font={small},         % キャプションは本文より少し小さく（論文の鉄則）
  labelfont={bf},       % ラベル（図1）は太字
  justification=centering % デフォルトは中央揃え
}
\captionsetup[figure]{justification=centerlast} % 図の説明が長い場合は左寄せ・最後だけ中央

% =============================================
% 5. ドキュメント情報
% =============================================
\title{\textbf{4. PIDによる温度制御}}
\author{電気電子工学科 5年 234 氏名 栁原魁人} 
\date{2025年12月4日}
