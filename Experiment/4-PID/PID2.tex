...
比例ゲイン $K_P$ を 1 および 100 に設定した場合の温度変化をそれぞれ測定した．その後，3分以内に目標温度 $\pm \SI{10}{\degreeCelsius}$ の範囲に収束する最適な $K_P$ を試行錯誤により決定した．

\subsubsection{PI制御}
$K_P$ を前項で決定した値に固定し，$T_D = 0$ としてPI制御を行った．
積分時間 $T_I$ を 1 および 100 に設定した場合の応答を確認した後，3分以内に目標温度 $\pm \SI{0.5}{\degreeCelsius}$ となる最適な $T_I$ を決定した．

\subsubsection{PID制御}
$K_P, T_I$ を前項までに決定した値とし，PID制御を行った．
微分時間 $T_D$ を 1 および 100 に設定した場合の応答を確認し，2分以内に目標温度 $\pm \SI{0.5}{\degreeCelsius}$ となる最適な $T_D$ を決定した．
...
本実験で観測されたP制御，PI制御，PID制御の応答波形を図Xに示す．
