...
% 理論的導出は簡潔に示す: 山–山/谷–谷 を比率 R として m=(R-1)/(R+1) を用いる。

% 注意: 数式内に直接 Unicode のダッシュ(–/—)を入れると描画に問題が出ることがあります。
%       数式中のハイフンは ASCII の '-' または $-$, 日本語文字を含む場合は \text{-} あるいは
%       \text{--} (二重ハイフン) を使ってください。
...
\begin{figure}[H]
  \centering
  % Use the provided SVG for frequency response instead of drawing with pgfplots
...
\begin{figure}[H]
  \centering
  % Use the provided SVG for input-output characteristic instead of plotting with pgfplots
...
\subsubsection*{実験結果}
\begin{itemize}
  \item AF IN: p-p = \SI{0.438}{\volt}, T = \SI{335}{\micro\second} →
    $f_{AFIN} \simeq \SI{2985}{\hertz} \simeq \SI{2.99}{\kilo\hertz}$.
  \item DET OUT: p-p = \SI{0.468}{\volt}, T = \SI{332}{\micro\second} →
    $f_{DETOUT} \simeq \SI{3012}{\hertz} \simeq \SI{3.01}{\kilo\hertz}$.
  \item AM OUT の包絡線（観測値）: PK--PK = \SI{4.26}{\volt}, 山--谷 = \SI{1.18}{\volt}, 谷--谷 = \SI{1.85}{\volt}.
  \item 変調度: 山--山 = \SI{4.26}{\volt}, 谷--谷 = \SI{1.85}{\volt} →
    $R=\dfrac{\text{山--山}}{\text{谷--谷}} = \dfrac{4.26}{1.85} \simeq 2.303$,
    直接に計算すると
    \( m = \dfrac{A - B}{A + B} = \dfrac{4.26 - 1.85}{4.26 + 1.85} \simeq 0.395 \simeq 39.5\% \).
\end{itemize}
\end{document}