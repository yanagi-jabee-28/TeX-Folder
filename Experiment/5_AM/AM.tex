...
実験(2)の波形（図\ref{fig:AFIN_waveforms}～図\ref{fig:DETOUT_waveforms}）を簡潔にまとめる．AF IN と DET OUT の周期はそれぞれ $T=\SI{335}{\micro\second}$, $T=\SI{332}{\micro\second}$ で周波数はいずれも約 \SI{3.0}{\kilo\hertz} に一致し，復調波形に顕著な歪みはない．包絡線の最大値 $A=\SI{4.26}{\volt}$，最小値 $B=\SI{1.85}{\volt}$ から
\[
 m=\frac{A-B}{A+B}=\frac{4.26-1.85}{4.26+1.85}\simeq 0.395
\]
変調度は約 40\% で，過変調は生じていない．

\subsection*{2. 実験(3) 周波数特性の考察}
図\ref{fig:freq_response_corrected} では \SI{2}{\kilo\hertz}～\SI{20}{\kilo\hertz} でほぼ一定出力。低域の減少は結合/バイパス用コンデンサで低周波が通りにくいため，高域低下は同調回路とトランスの限界による。音声帯域 (約 \SI{300}{\hertz}～\SI{3.4}{\kilo\hertz}) を扱うには十分平坦である。

\subsection*{3. 実験(4) 入出力特性の考察}
図\ref{fig:io_characteristic_corrected} では AF IN に対し AM OUT と DET OUT がほぼ比例し原点近くを通る。変調部は入力振幅に素直に追従し，ダイオードは十分な振幅で直線動作している。飽和や顕著な歪みは観測されない。
