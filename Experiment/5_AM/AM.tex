...
図\ref{fig:freq_response_corrected} では \SI{2}{\kilo\hertz}～\SI{20}{\kilo\hertz} でほぼ一定出力。低域の減少は結合/バイパス用コンデンサで低周波が通りにくいため，高域低下は同調回路とトランスの限界による。音声帯域 (約 \SI{300}{\hertz}～\SI{3.4}{\kilo\hertz}) を扱うには十分平坦である。\cite{tanno1988}

\subsection*{3. 実験(4) 入出力特性の考察}
図\ref{fig:io_characteristic_corrected} では AF IN に対し AM OUT と DET OUT がほぼ比例し原点近くを通る。変調部は入力振幅に素直に追従し，ダイオードは十分な振幅で直線動作している。飽和や顕著な歪みは観測されない。\cite{tanno1988}
