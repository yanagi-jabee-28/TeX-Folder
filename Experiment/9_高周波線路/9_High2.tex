...
\usepackage{graphicx}
% 画像検索パス
% 最適化画像があれば優先して参照する（scripts/optimize-images.ps1 を実行して作成）
% 使い方（リポジトリルートで）: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\optimize-images.ps1`
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
        \begin{axis}[width=\linewidth, xlabel={位置 $x$ (mm)}, ylabel={短絡 (dB/V)}, grid=both, unbounded coords=jump]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
        \begin{axis}[width=\linewidth, xlabel={位置 $x$ (mm)}, ylabel={解放 (dB/V)}, grid=both, unbounded coords=jump]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
        \begin{axis}[width=\linewidth, xlabel={位置 $x$ (mm)}, ylabel={整合 (dB/V)}, grid=both, unbounded coords=jump]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
        \begin{axis}[width=\linewidth, xlabel={位置 $x$ (mm)}, ylabel={半端もの (dB/V)}, grid=both, unbounded coords=jump]
    ...
    \caption{各終端条件における位置 $x$ に対する測定値（単位: dB/V）}
...
\begin{enumerate}
    ...
    \begin{equation}
        v = f \lambda = \SI{1.0e9}{Hz} \times \SI{0.170}{m} = \SI{1.70e8}{m/s}
    \end{equation}
    ...
    電圧定在波比 $\rho$（SWR）は電圧の比で定義される。
    \begin{equation}
        \mathrm{SWR} = \frac{V_{\mathrm{max}}}{V_{\mathrm{min}}}
    \end{equation}

    電圧を dB 表示に変換する式 $V_{(\mathrm{dB})} = 20\log_{10} V$ を用いると，最大値と最小値の差は
    \begin{align}
        V_{\mathrm{max}(\mathrm{dB})} - V_{\mathrm{min}(\mathrm{dB})} &= 20\log_{10}V_{\mathrm{max}} - 20\log_{10}V_{\mathrm{min}} \\
        &= 20\log_{10}\left(\frac{V_{\mathrm{max}}}{V_{\mathrm{min}}}\right) = 20\log_{10}(\mathrm{SWR})
    \end{align}

    これを SWR について解くと，
    \begin{equation}
        \log_{10}(\mathrm{SWR}) = \frac{V_{\mathrm{max}(\mathrm{dB})} - V_{\mathrm{min}(\mathrm{dB})}}{20},\qquad
        \mathrm{SWR} = 10^{\frac{V_{\mathrm{max}(\mathrm{dB})} - V_{\mathrm{min}(\mathrm{dB})}}{20}}.
    \end{equation}

    実測値 $V_{\mathrm{max}(\mathrm{dB})} \approx \SI{12.0}{dB\mu V}$，$V_{\mathrm{min}(\mathrm{dB})} \approx \SI{6.5}{dB\mu V}$ を代入すると，
    \begin{equation}
